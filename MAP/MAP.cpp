...
MyItem::MyItem(int x, int y, int f) : QGraphicsRectItem(x - 3, y - 3, 7, 7), cx(x), cy(y), cf(f), cs(0) {
    ...
    QBrush qbrush(f == 1 ? QColor(138, 43, 226) : QColor(0, 0, 255));   //根据f设置颜色
...
void MyScene::clickmouse(int x, int y, int lr) {
    if (lr == 1)
    {
        depart = true;
    }
    ...
    QGraphicsScene::addItem(new MyItem(x, y, lr));
}
/////////////////////////////////////////////////////////////////////////////////////
...
void MAP::findstop() {
    if (ui.action_search->isChecked() == false) return; //鼠标点击触发两次，第二次触发直接返回
    ui.action_search->setChecked(false);  //鼠标第一次触发设置状态为false,防止第2次触发进入
    if (se != Q_NULLPTR) {  //如果先前打开过站点及线路输入窗口
        se->show();         //则直接显示该窗口
        return;
    }
    arrive = depart = false;  //此时未选取步行起点或终点
    se = new QtWidgetsSE(); //如果以前没有打开过站点及线路输入窗口
    se->readloc();
    se->setWindowFlags(Qt::WindowStaysOnTopHint); //设置最外层显示
    se->myShow(ui.graphicsView);
}
...
void MAP::leasttran() {
    ...
    if (ui.action_leasttran->isChecked() == false) return; //鼠标点击一次触发两次，第二次触发直接返回
    ui.action_leasttran->setChecked(false);  //鼠标第一次触发设置状态为false,防止第2次触发进入
    ...
    QBrush qbrush(QColor(0, 107, 60));//设置颜色
    QPen qpens(qbrush, 4, Qt::SolidLine, Qt::SquareCap, Qt::BevelJoin); //站点的笔
    QPen qpenl(qbrush, 3, Qt::SolidLine, Qt::SquareCap, Qt::BevelJoin); //线路的笔
...
void MAP::mindist() {
    ...
    if (ui.action_mindist->isChecked() == false) return; //鼠标点击一次触发两次，第二次出发直接返回
    ui.action_mindist->setChecked(false);  //鼠标第一次出发，设置状态为false,防止第2次出发进入
    QList<QGraphicsItem*> listItem;
    ...
    QGraphicsLineItem* myLItem;
    MyScene* scene = (MyScene*)(ui.graphicsView->scene());
    QBrush qbrush(QColor(0, 255, 246));//设置颜色
    QPen qpens(qbrush, 4, Qt::SolidLine, Qt::SquareCap, Qt::BevelJoin); //站点的笔
    QPen qpenl(qbrush, 3, Qt::SolidLine, Qt::SquareCap, Qt::BevelJoin); //线路的笔
    route = true;                       //进入查询路径显示期间，不响应步行起点与终点选取
    int c = 0;                          //c为可行线路数
